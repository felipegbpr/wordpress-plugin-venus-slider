    ...
    class VenusSliderScripts {
                ...
                <script type="text/javascript">
                    jQuery(document).ready(function ($) {
                        ...
                        $('body').find('.venus-slider').each(function () {
                            let _this = $(this);
                            let isVideo = _this.data('slide-type') == 'video-carousel' ? true : false;
                            let videoWith = isVideo ? _this.data('video-width') : false;
                            let videoHeight = isVideo ? _this.data('video-height') : false;
                            let autoWidth = isVideo ? true : false;

                            if (jQuery().magnificPopup) {
                                let popupType = _this.data('slide-type') == 'product-carousel' ? 'ajax' : 'image';
                                let popupGallery = _this.data('slide-type') != 'product-carousel' ? true : false;
                            ...
                            if (jQuery().owlCarousel) {
                                _this.owlCarousel({
                                    ...
                                    responsive: {
                                        320: {items: _this.data('columns-mobile')},
                                        600: {items: _this.data('columns-small-tablet')},
                                        768: {items: _this.data('columns-tablet')},
                                        993: {items: _this.data('columns-small-desktop')},
                                        1200: {items: _this.data('columns-desktop')},
                                        1921: {items: _this.data('columns')}
                                    }
                                });
                            }
                        })
        ...
        private function should_load_scripts() {
            ...
            $load_scripts = is_active_widget( false, false, 'widget_venus_slider', true ) ||
                            ( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'venus_slider' ) ) ||
