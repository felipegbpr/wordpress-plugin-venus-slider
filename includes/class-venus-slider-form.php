...
if ( ! class_exists( 'VenusSliderForm' ) ) :

    class VenusSliderForm {
        ...
        public function images_sizes( array $args ) {
            ...
            foreach ( get_intermediate_image_sizes() as $_size ) {
                if ( in_array( $_size, array( 'thumbnail', 'medium', 'medium_large', 'large' ) ) ) {
                    ...
                    $width = $_wp_additional_image_sizes[ $_size ]['crop'] ? 'hard' : 'soft';
        ...
        public function file( array $args ) {
            ...
            echo $this->field_before( $args );
            echo sprintf( '<input type="text" class="sp-input-text" value="%1$s" id="%2$s" name="%3$s">', $value, $args['id'], $name );
            echo sprintf( '<input type="button" class="button" id="venus_slider_video_btn" value="%s">', __( 'Browse', 'venus-slider' ) );
            echo $this->field_after();
        }
        ...
        private function field_common( $args ) {
            global $post;
            // Meta name
            $group    = isset( $args['group'] ) ? $args['group'] : 'venus_slider';
