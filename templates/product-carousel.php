...
if ( ! defined( 'WC_VERSION' ) ) {
	if ( current_user_can( 'manage_options' ) ) {
		echo sprintf( esc_html__( 'Venus Slider needs %s to work for products carousel.', 'venus-slider' ), sprintf( '<a href="https://wordpress.org/plugins/woocommerce/" target="_blank" >%s</a>', __( 'WooCommerce', 'venus-slider' ) ) );
	}
	...
	<div class="product venus-slider__product">
		...
		if ( has_post_thumbnail( $post->ID ) ) {
			if ( $_lazy_load_image == 'on' ) {
				...
				$image_src = get_the_post_thumbnail( $post->ID, $_image_size );
